...
class ScrewSupport(BasePartObject):
    def __init__(self, rotation: tuple[float, float, float] | Rotation = (0, 0, 0), align: Align | tuple[Align, Align, Align] = None, mode: Mode = Mode.ADD):
        with BuildPart() as p:
            ...
            fillet(edges(Select.LAST).filter_by(Axis.Z).group_by(Axis.X)[-1].sort_by(Axis.Y)[-1],
            ...
            fillet(edges().filter_by(Plane.XY).group_by(Axis.Z)[-1], radius=0.25)
...
class BaseSquare(BasePartObject):
    def __init__(self, rotation: tuple[float, float, float] | Rotation = (0, 0, 0), align: Align | tuple[Align, Align, Align] = None, mode: Mode = Mode.ADD):
        with BuildPart() as p:
            ...
            with Locations(vertices().group_by(Axis.Z)[0].group_by(Axis.X)[-1].sort_by(Axis.Y)[-1]):
                ScrewSupport(align=(Align.MIN, Align.MIN, Align.MIN), rotation=(0, 0, 180))
                #fillet(edges(Select.NEW).filter_by(Axis.Z), radius=screw_support_fillet/3)
...
class BaseGrid(BasePartObject):
    def __init__(self, x, y, rotation: tuple[float, float, float] | Rotation = (0, 0, 0), align: Align | tuple[Align, Align, Align] = None, mode: Mode = Mode.ADD):
        with BuildPart() as p:
...
class ClipEdge(BasePartObject):
    def __init__(self, len, rotation: tuple[float, float, float] | Rotation = (0, 0, 0), align: Align | tuple[Align, Align, Align] = None, mode: Mode = Mode.ADD):
        with BuildPart() as p:
